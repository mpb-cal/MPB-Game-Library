...
const char*		PARTICLE_TEXTURE				= "GeneratedParticleTexture";
	...
	const char*		SINGLE_TEXTURE				= "planeTexture.tex";
#else
...
GLfloat 								MPBOpenGL::m_perspectiveFar = 2000;
...
MPBVectorList						MPBOpenGL::m_drawPoints;
...
//	MPB_ASSERT( glFogCoordd );
	...
	strcpy( m_fontTexture, "font2.tex" );
...
//				buf[ strlen( buf ) - 4 ] = 0;
				createTextureFromFile( texId, dirbuf, buf, 255, mipmap );
		...
		sprintf( dirbuf, "%s/alpha", m_textureDir, i );
		sprintf( buf, "%s/*.bmp", dirbuf, i );
		WIN32_FIND_DATA findData;
		HANDLE handle = FindFirstFile( buf, &findData );
		...
		while (handle != INVALID_HANDLE_VALUE)
		{
			...
			MPBTexture* t = new MPBTexture( findData.cFileName, 255, true, GL_REPEAT );
			m_textures[0].push_back( t );

			char buf[1000];
			sprintf( buf, "%s/%s", dirbuf, findData.cFileName );
			t->loadFromBMPAlphaFile( buf );
...
void MPBOpenGL::drawPoint( MPBVector p )
{
...
void MPBOpenGL::drawStencilShadows( bool objects, bool planes )
{
	...
	glStencilFunc( GL_NOTEQUAL, 0, ~0 );	// only draw where the stencil was set (!=0)
...
//	if (MPBOpenGL::is2D()) mipmap = false;
	if (!strcmp( name, "health.tex" )) mipmap = false;
	if (!strcmp( name, "healthSign.tex" )) mipmap = false;
	if (!strcmp( name, "breath.tex" )) mipmap = false;
	if (!strcmp( name, "breathSign.tex" )) mipmap = false;
	if (!strcmp( name, "crosshairs.tex" )) mipmap = false;
	if (!strcmp( name, "font2.tex" )) mipmap = false;
...
// returns true if any point is in frustum
...
void MPBOpenGL::reflectAboutPlane( Plane* p )
{
	...
	glTranslatef( T.x, T.y, T.z );		// T
	glMultMatrixf( N );						// N
	glScalef( 1, 1, -1 );					// M
	glMultMatrixf( N_inv );					// N'
	glTranslatef( -T.x, -T.y, -T.z );	// T'
}
